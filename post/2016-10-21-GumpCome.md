...
GumpCome want to use Power BI to present their device status data, due to Power BI's excellent graph and easy to use for business operator. Because Power BI isn't GA in china ,so we can't let SA directly connect with Power BI. So GumpCome team store the data from SA to SQL Azure, and using Power BI connect to SQL Azure. 
